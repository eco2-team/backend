"""OpenTelemetry Distributed Tracing Configuration for Chat API.

분산 트레이싱 설정:
- FastAPI 자동 계측 (HTTP 요청/응답)
- HTTPX 자동 계측 (외부 API 호출)
- Redis 자동 계측 (Pub/Sub, 캐시)

Architecture:
  Chat API (OTel SDK) -> OTLP/HTTP (4318) -> Jaeger Collector
"""
