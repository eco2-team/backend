...
class TestOAuthAuthorizeInteractor:
    ...
    @pytest.fixture
    def mock_oauth_state_store(self) -> AsyncMock:
        return AsyncMock()

    @pytest.fixture
    def mock_oauth_provider(self) -> MagicMock:
        provider = MagicMock()
        provider.get_authorization_url.return_value = "https://accounts.google.com/oauth?..."
        return provider

    @pytest.fixture
    def interactor(
        self,
        mock_oauth_state_store: AsyncMock,
    ...
    async def test_execute_creates_state_and_returns_url(
        ...
        # oauth_state_store.save가 호출되었는지 확인
        mock_oauth_state_store.save.assert_called_once()
        call_args = mock_oauth_state_store.save.call_args
    ...
    @pytest.mark.asyncio
    async def test_execute_calls_oauth_provider_with_correct_params(
        self,
        interactor: OAuthAuthorizeInteractor,
        mock_oauth_provider: MagicMock,
    ) -> None:
        """OAuth 프로바이더에 올바른 파라미터 전달 테스트."""
        # Arrange
        request = OAuthAuthorizeRequest(
            provider="google",
            redirect_uri="http://localhost:8000/callback",
        ...
        call_kwargs = mock_oauth_provider.get_authorization_url.call_args[1]
