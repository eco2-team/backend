...
```
platform/helm/node-lifecycle-operator/
├── Chart.yaml
├── values.yaml
├── templates/
│   ├── deployment.yaml
...
```yaml
# argocd/apps/30-platform.yaml (Node Lifecycle Operator 예시)
...
spec:
  ...
  source:
    ...
    targetRevision: main
    path: platform/helm/node-lifecycle-operator
    helm:
      valueFiles:
        - values.yaml
...
```bash
# Operator 설치
kubectl apply -f argocd/apps/30-platform.yaml
