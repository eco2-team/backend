...
│   │   └─ {base,dev,prod}/application.yaml
│   └─ ...
├─ crds/
│   ├─ alb-controller/
...
파트별 공통 `platform/helm/<component>/<env>/application.yaml` 템플릿:
...
- **Manifest 위치**:  
  - dev: `platform/helm/alb-controller/dev/application.yaml`  
  - prod: `platform/helm/alb-controller/prod/application.yaml`  
...
- **Manifest 위치**: `platform/helm/kube-prometheus-stack/{dev,prod}/application.yaml`
...
- **Manifest 위치**: `platform/helm/postgres-operator/{dev,prod}/application.yaml`
...
- **Manifest 위치**: `platform/helm/redis-operator/{dev,prod}/application.yaml`
- Operator만 Helm으로 설치하고, 실제 Redis CR은 `platform/cr/{base,dev,prod}` 디렉터리에서 정의한다.

- **Manifest 위치**: `platform/helm/external-secrets/{dev,prod}/application.yaml`
