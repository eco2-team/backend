...
| 25 | [Redis 3-Node 클러스터 프로비저닝](./25-redis-3node-cluster-provisioning.md) | ✅ 완료 | 단일→3노드 분리, Terraform/Ansible, vCPU 트러블슈팅 |
| 26 | [Spotahome Redis Operator 배포](./26-redis-operator-deployment.md) | ✅ 완료 | RedisFailover CR, Helm Chart, ArgoCD 연동 |
| 27 | [KEDA 이벤트 드리븐 오토스케일링](./27-keda-event-driven-autoscaling.md) | ✅ 완료 | RabbitMQ ScaledObject, Prometheus Adapter, k6 튜닝 |
...
| Redis Operator | Spotahome Redis Operator | 1.2.4 | Sentinel HA |
| GitOps | ArgoCD | 2.13+ | |
| Autoscaling | KEDA | 2.12.1 | 이벤트 드리븐 스케일링 |
| Metrics | Prometheus Adapter | 4.3.0 | 커스텀 메트릭 HPA |
...
- [x] Redis 3-Node 클러스터 프로비저닝 (Auth, Streams, Cache 분리)
- [x] Spotahome Redis Operator 배포
- [x] KEDA 이벤트 드리븐 오토스케일링 (scan-worker)
- [x] Prometheus Adapter 커스텀 메트릭 (scan-api)
