...
| --- | --- | --- | --- | --- |
| -1 | Security Foundation | OPA/Kyverno, External Secrets, (옵션) cert-manager | Helm+Kustomize 혼합 | `k8s/security`, `platform/helm/security/*` (planned) |
...
| 40 | Observability | kube-prometheus-stack, Grafana | Helm | `platform/helm/observability/kube-prometheus-stack` |
| 60 | Data Layer | PostgreSQL, Redis, RabbitMQ | Helm | `platform/helm/data/<component>` |
| 70 | GitOps Tools | Atlantis | Helm | `platform/helm/platform/atlantis` |
...
│       ├── 30-platform.yaml
│       ├── 40-monitoring.yaml     # Helm → platform/helm/observability/…
...
spec:
  ...
  source:
    repoURL: https://github.com/SeSACTHON/backend
    targetRevision: develop
    path: platform/helm/observability/kube-prometheus-stack
    helm:
...
2. **Repo 구조 복원**  
   ...
   - `platform/helm/observability/*` 처럼 Helm 모듈을 `platform/helm/` 아래에 배치하고, `k8s/` 는 Kustomize 전용으로 유지.
3. **ArgoCD에 등록**  
