...
```mermaid
graph TB
    subgraph EKS["AWS EKS 클러스터"]
        CP[Control Plane<br/>$73/월]
        
        subgraph NodeGroup1["API Nodes"]
            N1[t3.medium × 3<br/>$30 × 3 = $90/월]
        end
        
        subgraph NodeGroup2["Worker Nodes"]
            N2[t3.large × 2<br/>$60 × 2 = $120/월]
        end
    end
    
    subgraph Add["부가 서비스"]
        ECR[ECR<br/>50GB<br/>$5/월]
        ALB[ALB<br/>$16/월]
        EBS[EBS<br/>100GB<br/>$10/월]
        DataTransfer[데이터 전송<br/>100GB<br/>$9/월]
    end
    
    subgraph Free["무료 컴포넌트"]
        Argo[ArgoCD<br/>$0<br/>EKS 내 실행]
        Helm[Helm<br/>$0<br/>클라이언트 도구]
...
```mermaid
flowchart LR
    A[Week 1<br/>EKS 구축<br/>최소 비용 모드] --> B[Week 2-3<br/>개발<br/>Docker Compose]
    B --> C[Week 4<br/>EKS로 이전<br/>테스트]
    C --> D{해커톤 D-1}
...
```mermaid
gantt
    title EKS 구축 및 개발 일정
    dateFormat YYYY-MM-DD
    
    section 인프라
    EKS 클러스터 생성          :2025-11-01, 4h
    ArgoCD 설치               :2025-11-01, 2h
    Helm Charts 작성          :2025-11-02, 6h
    테스트 & 검증              :2025-11-03, 4h
    
    section 개발
    서비스 개발 (병렬)         :2025-11-04, 14d
    EKS 통합 테스트            :2025-11-18, 3d
    부하 테스트                :2025-11-21, 2d
    
    section 백업
    Docker Compose 유지        :2025-11-04, 20d
    최종 검증                  :2025-11-24, 2d
    
    section 해커톤
    배포 환경 선택             :crit, 2025-11-26, 1d
    데모 시연                  :milestone, 2025-11-27, 0d
```
