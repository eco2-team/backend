...
| 2 | Observability | kube-prometheus-stack, Grafana | Helm | `charts/observability/kube-prometheus-stack` |
| 3 | Data Layer | PostgreSQL, Redis, RabbitMQ | Helm | `charts/data/<component>` (작성 중) |
| 4 | Platform Services | Storage worker, AI worker, Atlantis | Kustomize/Helm 혼합 | `k8s/platform/*`, `charts/platform/*` |
| 5 | Progressive Delivery | Argo Rollouts, Service Mesh | Helm/Kustomize | `k8s/progressive-delivery` (planned) |
| 6 | API Services | auth, my, scan, character, ... | Kustomize overlays | `k8s/overlays/<domain>` (재편 중) |
...
│       ├── 20-platform.yaml
│       ├── 30-monitoring.yaml     # Helm → charts/observability/…
│       ├── 40-data-operators.yaml
│       ├── 50-data-clusters.yaml
│       ├── 60-gitops-tools.yaml
│       └── 70-apis-app-of-apps.yaml
├── k8s/
│   ├── infrastructure/
│   ├── namespaces/
│   ├── networkpolicies/
...
- 이 문서(`docs/architecture/APP-OF-APPS-DECISION.md`)와 Wave별 운영 책임 문서를 업데이트해 신규 팀 온보딩을 돕는다.
