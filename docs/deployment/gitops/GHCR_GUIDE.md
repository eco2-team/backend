...
| 구성 요소 | 형식 | 예시 |
|-----------|------|------|
| API 서비스 | `{service}-{version}-{sha6}` | `auth-0.7.3-8255d8` |
| GHCR 주소 | `ghcr.io/sesacthon/{service}-api:{tag}` | `ghcr.io/sesacthon/auth-api:auth-0.7.3-8255d8` |

- **`version`**: 현재 클러스터 기준 `0.7.3`
- **`sha6`**: 해당 커밋의 `git rev-parse --short=6 HEAD`
- `latest` 태그는 유지하되, ArgoCD/Kustomize는 위 패턴의 불변 태그를 사용해 롤백이 가능하도록 합니다.

CI 단계에서는 서비스에 변경이 감지되면:
1. `{service}-{version}-{sha6}` 태그로 이미지를 빌드/푸시하고
2. `k8s/overlays/{service}/patch-deployment.yaml` 의 이미지 값을 동일하게 갱신합니다.
