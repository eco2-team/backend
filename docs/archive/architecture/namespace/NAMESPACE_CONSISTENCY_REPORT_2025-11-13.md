...
- [x] `relabelings`에 `namespace`, `domain`, `phase`, `tier` 자동 추가 설정
...
### Tier 구조 (Application-Centric 4-Tier Architecture)
```
┌─────────────────────────────────────────────────────────────┐
...
┌─────────────────────────────────────────────────────────────┐
│ Tier 0: Observability & Infrastructure                      │
│ - monitoring (Tier: observability)                          │
│ - atlantis (Tier: infrastructure)                           │
└─────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────┐
│ Tier 2: Business Logic (Tier: business-logic)               │
│ - auth, my, scan, character, location, info, chat          │
└─────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────┐
│ Tier 3: Integration (Tier: integration)                     │
│ - messaging (RabbitMQ - async)                              │
└─────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────┐
│ Tier 4: Data (Tier: data)                                   │
...
### ✅ 모든 불일치 사항 수정 완료
- **Kubernetes Manifests**: NetworkPolicy, ServiceMonitor tier 레이블 수정
- **Kustomize Overlays**: 7개 도메인 모두 데이터베이스 연결 문자열 수정 (`.db.` → `.data.`, `.messaging.`)
- **ArgoCD ApplicationSet**: tier 레이블 수정 (`api` → `business-logic`)
- **Ansible**: `postgres_namespace`, `redis_namespace` 수정 (`db` → `data`)
