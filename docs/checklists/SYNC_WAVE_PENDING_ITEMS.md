...
- **조치**:
  1. ExternalDNS Helm values(dev/prod)에 `--annotation-filter=external-dns.alpha.sesacthon.io/managed-by in (external-dns)`를 추가해, 해당 annotation이 있는 리소스만 자동 관리하도록 제한.
...
- **조치 완료**: Argo CD, Kustomize, Helm, Ansible 매니페스트 전반에서 `kubernetes.io/` 라벨/annotation prefix를 `sesacthon.io/`로 일괄 교체해 노드/워크로드 라벨 체계를 통일했다.
- **추가 액션**:
