...
이코에코(Eco²)는 AI 캐릭터 이코(Eco)와 함께 올바른 분리배출을 실천할 수 있도록 돕는 AI 기반의 분리수거 안내 앱입니다. <br>
사용자의 참여율과 분리배출 정확도를 동시에 높이는 생활밀착형 행동 코칭 서비스를 지향합니다. <br>
Self-managed Kubernetes를 기반으로 AI 폐기물 분류·지도·챗봇 등 도메인 API와 데이터 계층, GitOps 파이프라인을 단일 리포지토리에서 관리합니다. <br>
...
graph TD
    ...
    RABBITMQ["RabbitMQ"] --> WORK["Worker Pods"]
    WORK["Worker Pods"] --> POSTGRE["PostgreSQL"]
