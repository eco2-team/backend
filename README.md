...
| **Chat 세션 저장소로 인해 Classification 지연** | Redis + Postgres에 history를 저장하느라 Vision/Text 파이프라인이 실행되지 않거나 지연됨 → 세션 로직·DB 테이블 제거 후 stateless API로 단순화 | `docs/troubleshooting/2025-12-02-v1.0.0.md#5-chat-세션-저장소-제거로-classification-파이프라인-복구` |
