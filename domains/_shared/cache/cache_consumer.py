...
from domains._shared.cache.character_cache import CharacterLocalCache, get_character_cache
...
class CacheUpdateConsumer(ConsumerMixin):
    ...
    fanout exchange를 사용하므로 모든 Worker가 동일한 이벤트를 수신합니다.
    각 Worker는 고유한 임시 큐를 생성하여 exclusive하게 수신합니다.
    ...
    def __init__(self, connection: Connection, cache: CharacterLocalCache | None = None):
        """Consumer 초기화.
        ...
        # 각 worker마다 고유한 임시 큐 (exclusive, auto_delete)
        self.queue = Queue(
...
class CacheConsumerThread(threading.Thread):
    ...
    Worker 시작 시 이 스레드를 시작하여 캐시 동기화를 유지합니다.
    ...
    def __init__(self, broker_url: str, cache: CharacterLocalCache | None = None):
        """Thread 초기화.
...
def start_cache_consumer(broker_url: str) -> CacheConsumerThread:
