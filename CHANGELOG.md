...
### Added
- **RabbitMQ Operator + Celery 비동기 처리 도입**
  - **RabbitMQ Operator**: Cluster Operator + Messaging Topology Operator ArgoCD 통합
  - **Topology 선언적 관리**: Vhost, Queue, Exchange, Binding을 CRD로 정의
  - **Celery Task Queue**: 4단계 Chain Pipeline (Vision → Rule → Answer → Reward)
  - **SSE Progress Streaming**: 실시간 작업 진행 상황 클라이언트 전송
  - **Worker 분리**: scan-worker (AI 처리), character-worker (보상/DB), celery-beat (스케줄러)

- **Async Scan API**
  - `POST /api/v1/scan/classify/async`: 비동기 분류 요청
  - `GET /api/v1/scan/{task_id}/progress`: SSE 진행 상황 스트리밍
  - Celery Events 기반 실시간 상태 추적

### Changed
- **Worker 아키텍처**
  - scan-worker: worker-ai 노드 배치, AI 파이프라인 전담
  - character-worker: worker-storage 노드 배치, 보상 판정 및 DB 저장
  - celery-beat: DLQ 재처리 스케줄링
- **Network Policy 확장**
  - `allow-rabbitmq-egress`: scan/character → rabbitmq AMQP(5672) 허용
  - `allow-rabbitmq-access`: rabbitmq-system 15672 Management API 접근 허용

### Fixed
- **CI 파이프라인**: 멀티라인 커밋 메시지 SIGPIPE 에러 수정
- **pydantic-settings v2 호환성**: list 타입 환경변수 빈 문자열 파싱 에러 해결
- **Dockerfile**: character-api에 `_shared` 모듈 복사 추가
...
### Added
- **EFK 로깅 스택 구축**
  - **Elasticsearch**: ECK Operator 기반 3노드 클러스터
  - **Fluent Bit**: DaemonSet 기반 로그 수집, CRI 파서 지원
  - **Kibana**: 선언적 DataView/Dashboard 관리
  - **ECS 구조화 로깅**: 표준 필드 네이밍, PII 마스킹

- **분산 트레이싱 강화**
  - **OpenTelemetry Auto-Instrumentation**: 모든 FastAPI 서비스에 적용
  - **B3/W3C Propagator**: Istio-App 간 trace context 전파
  - **ext-authz 트레이싱**: gRPC 메타데이터에서 trace context 추출

- **Alerting 시스템**
  - **Alertmanager + Slack**: 인프라/서비스 알림 통합
  - **Pod 상태 리포트**: 스케줄 기반 Slack 보고
  - **Green Resolved**: 복구 알림 지원
...
### Changed
- **gRPC 도메인 간 통신**
  - character ↔ my: gRPC 기반 캐릭터 소유 정보 동기화
  - Circuit Breaker, Retry Policy 적용
- **코드 품질 개선**
  - character, my, location, image, chat, scan 도메인 리팩토링
  - Strategy 패턴, Cache, 테스트 추가
  - Dead code 제거, 상수 정리

### Fixed
- **Jaeger OOMKilled**: 메모리 설정 최적화
- **Fluent Bit CRI Parser**: containerd 로그 형식 지원
- **Network Policy**: Prometheus scraping, Zipkin egress 허용
- **ext-authz**: CORS preflight 우회, case-insensitive 헤더 처리
